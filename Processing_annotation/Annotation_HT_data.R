...
path = '~/Spatial_atlas/Datasets/Merfish/Human_tonsil/'
Output_path = '~/Spatial_atlas/Picture_pipeline/Merfish/Human_tonsil/'
Method = "Merfish"
Tissue = "Human_tonsil"
#-------------------------------------------------------------------------------------------------------
# Data loading 
Expression_file = read.csv(paste(path, "Merfish_Human_tonsil_ExprMat.csv", sep = ""), header = TRUE)
Meta_data = read.csv(paste(path,"Merfish_Human_tonsil_Metadata.csv",sep =""), header = TRUE) 
# In Meta_data, x and y coordinates should be named respectively 'cell_centroid_x' and 'cell_centroid_y'
colnames(Meta_data)[c(4,5)] <- c("cell_centroid_x", "cell_centroid_y")
# From here, Expression_file should be numeric and count matrix only, cell ID column is deleted
Expression_file = Expression_file[,-c(1)]
...
Data_curated = Curate_data(Expression_file,Meta_data,QC2_results,min_lib_size=10,max_lib_size=2500,min_cell_radius=2,max_cell_radius=8)
